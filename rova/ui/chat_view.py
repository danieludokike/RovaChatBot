from PySide6.QtWidgets import QWidget, QVBoxLayout, QScrollArea, QFrame, QSizePolicy, QLabel
...
class ChatView(QWidget):
    def __init__(self):
        ...
        self.inner.setContentsMargins(16,16,16,16); self.inner.setSpacing(8)
        self.inner.addStretch(1)

        self.scroll.setWidget(self.card)
        root.addWidget(self.scroll)

    def add_message(self, text: str, role: str) -> QLabel:
        """Insert a message bubble and return its QLabel so caller can update text."""
        ...
        # return the inner label (index 0 of the bubble layout)
        return bubble.layout().itemAt(0).widget()
